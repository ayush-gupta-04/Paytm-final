...
export default function EmailForEmailVerification(){
    ...
    async function shootMailToEmail(data : emailFormat){
        setLoading(true)
        const res = await ShootMailForPassChange(data) as BackendResponse;
        setLoading(false);
        setResponse(res);
        resetField;
    }
        ...
        <div className="w-1/3 bg-white px-8 py-8 text-center flex flex-col gap-3 rounded-lg">
            ...
            <form className="flex flex-col" onSubmit={handleSubmit(shootMailToEmail)}>
                ...
                <div className="flex flex-row justify-between">
                    ...
                    <div className={`text-gray-600 hover:text-blue-600 font-medium hover:cursor-pointer mb-2 text-md ${response.success?"":"hidden"}`}
                    onClick={() => {router.push(`/forgotpass/verifyotp/${response.userIdToken}`)} }>
                        verify otp
                    </div>
                </div>
                <div className="mb-4">
