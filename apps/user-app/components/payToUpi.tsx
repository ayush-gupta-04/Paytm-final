...
export default function PayToUpiId(){
    ...
    return(
        <>
            <div className="size-60 hover:scale-105 cursor-pointer transition-all" onClick={() => {setStep('verify')}}>
                <img src="./payToUpi.png" />
            </div>
            <BackgroundSupporter hide = {step == null}></BackgroundSupporter>
            {step == "verify" && <VerifyUpiPopup onBack={handlePreviousStep} onSuccess={handleNextStep} step = {step} setStep = {setStep}></VerifyUpiPopup>}
...
function EnterTPINpopup({onSuccess,onBack,setStep,step} : {onSuccess : () => void,onBack : () => void,setStep : Dispatch<SetStateAction<string | null>>,step : string | null}){
    ...
    async function onFormSubmit(data : tpinFormat){
        ...
        setLoading(true);
        const res = await p2pTransferToUpi({upi : transferToUpi?.upi || "" , amount : transferToUpi?.amount || "" , tpin : data.tpin1 + data.tpin2 + data.tpin3 + data.tpin4 + data.tpin5 + data.tpin6}) as BackendResponseWithData;
        setResponse(res);
        setLoading(false);
        if(res.success){
