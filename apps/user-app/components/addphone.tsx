...
export default function AddPhone({serverPhone} : {serverPhone : string | null}){
    ...
    return(
        <div className="bg-white px-4 py-2 flex flex-col rounded-t-lg border-b-2 hover:bg-gray-100" onClick={() => {setHide(!hide)}}>
            ...
            <div className={`w-screen z-10 fixed top-1/2 left-1/2 transition-transform -translate-x-1/2 -translate-y-1/2 h-full duration-250 ${hide?"scale-y-0":"scale-y-100 "}`} onClick={() => {if(!loading){reset();setResponse({success : null,message : ""});setHide(!hide)}}}>
