    ...
    return (
        <div className="fixed rounded-lg w-1/3 flex flex-col z-20 top-1/2 left-1/2 transition-all -translate-x-1/2 -translate-y-1/2 bg-white px-4 py-4 shadow-xl" onClick={(e) => {e.stopPropagation()}}>
            ...
            <div className="flex flex-row gap-2">
                ...
                <button className="bg-red-500 hover:bg-red-600 w-full py-3 rounded-lg active:scale-95 transition-all text-center" onClick={async () => { await signOut({callbackUrl : "http://localhost:3000"})}}>Confirm Logout</button>
            </div>
        </div>
    )
}
