...
export default function VerifyPhonePopup({onSuccess,onBack,setStep,step} : {onSuccess : () => void,onBack : () => void,setStep : Dispatch<SetStateAction<string | null>>,step : string | null}){
    ...
    async function onFormSubmit(data : phoneFormat){
        ...
        if(response.success){
            ...
            setLoading(true);
            const res = await getDetailsOfPhone(data) as BackendResponse;
            setResponse(res);
            setLoading(false);
            if(res.success){
                setTransferToPhone({phone : data.phone,amount : null})
            }
        }
    }
    return(
        <div className="fixed z-20 top-1/2 left-1/2 transition-all -translate-x-1/2 -translate-y-1/2 w-1/3 bg-white shadow-black shadow-xl py-3 px-4 rounded-lg" onClick={(e) => {e.stopPropagation()}}>
            ...
            <form  onSubmit = {handleSubmit(onFormSubmit)} className="rounded-b-lg pt-4 flex flex-col gap-4">
                ...
                <div className="flex flex-row gap-2">
                    <div className="bg-slate-300 hover:bg-slate-400 w-full py-3 rounded-lg active:scale-95 transition-all text-center" aria-disabled = {loading} onClick={(e) => {setResponse({success : null,message : "",name : null});setStep(null);setTransferToPhone(null);e.stopPropagation()}}>Cancel</div>
                    <Button1 loading = {loading} text = {response.success?"Proceed":"Verify Phone"}></Button1>
                </div>
            </form>
        </div>
    )
